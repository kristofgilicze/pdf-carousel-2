...
<template>
  ...
  <main class="flex flex-col h-screen w-full justify-around">
    
    <div class="flex flex-col flex-grow gap-5 w-full justify-center items-center">
    ...
    <div class="flex items-stretch items-center justify-between">
      <button v-show="currentPage != 1" @click="navLeft" type="button" class=" flex  h-10 w-10">
        <svg class="h-6 w-6 dark:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"
          xmlns="http://www.w3.org/2000/svg">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16l-4-4m0 0l4-4m-4 4h18" />
        </svg>
      </button>
      <nav class="flex basis-1/2 items-center items-stretch">
        <OpenExternalButton :url="url" class="flex flex-grow items-stretch basis-1/4" />
        <input v-model="currentPage" type="number" :max="MAX"
          class="self-center flex-grow basis-1/4 self-strecth h-10 w-24 flex bg-gray-300 border border-gray-300 text-gray-900 text-lg text-center rounded-lg focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
          required />
        
      </nav>
      <button @click="navRight" type="button" class="h-10 w-10 ">
        <svg class="h-6 w-6 dark:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"
          xmlns="http://www.w3.org/2000/svg">
